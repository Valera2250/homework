class Product:
    ...
    def __str__(self):
        return f"{self.name} (Цена: {self.price} грн., Описание: {self.description}, Размеры: {self.dimensions})"
...
class Order:
    ...
    def __str__(self):
        products_info = "\n".join(
            [f"{product.name} x{quantity} - {product.price * quantity} грн." for product, quantity in self.items]
        )
        total_cost = self.calculate_total()
        return (
            f"Заказ для {self.customer}\n" +
            f"Товары:\n{products_info}\n" +
            f"Итого: {total_cost} грн."
        )
...
customer = Customer("Иван", "Иванов", "Иванович", "+3807777777")
